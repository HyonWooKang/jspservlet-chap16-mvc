...
<body>
	...
	<h3>EMPLOYEE 테이블에서 직원 전체 정보 조회(퇴사하지 않은 직원만 조회 할 것) URLMAPPING : /employee/list (get방식)</h3>
	<button onclick="location.href='${ pageContext.servletContext.contextPath }/employee/list'">
		직원정보 전체 조회하기
	</button>
	<!-- 서블릿명 : SelectAllEmpServlet, 응답 받는 페이지 주소명 : employeeList.jsp -->
	
	<h3>EMPLOYEE 테이블에서 신규 직원 정보 추가(필요한 정보를 입력받을 수 있는 form을 만들어줌) : /employee/insert (post)</h3>
	<!-- 서블릿: InsertEmpServlet, 페이지: views/common/successPage.jsp "성공메시지" -->
	<!-- 과정에 문제 발생함 => 강사님께 문의 -->
	<form action="${ pageContext.servletContext.contextPath }/employee/insert" method="post">
      직원명 : <input type="text" name="empName"><br> 
      주민등록번호 : <input type="text" ><br>
